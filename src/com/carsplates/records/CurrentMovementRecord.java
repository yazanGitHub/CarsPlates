 ...
 * --------------------------------------------------------------------------------
 *    represents the table  CURRENT_MOVEMENT in database.                        
 * --------------------------------------------------------------------------------
 * @version 2.0                                                                    
 * 
 */
 
 public class CurrentMovementRecord extends Record
 {
 
    //Constructor 
    public CurrentMovementRecord (  ConnectionContext connection ) 
    {
       super( "CURRENT_MOVEMENT" , connection ) ;
 ...
 @Override
    public void get( ResultSet rs , int type )
    {
       ...
       {
           logWriter.write(ex);
          System.out.println( "CurrentMovementRecord.get ( ResultSet rs , int type )" + ex.toString()); 
       }
 ...
 @Override
    public boolean insert ( int type )
    {
       ...
       try 
       {
          if ( m_pnr < 1 ) 
          {
             String SQL =  " INSERT INTO  CURRENT_MOVEMENT    ( PNR_PLATE , PNR_VEHICLE , PNR_VEHICLE_CARD , PNR_DELIVERED_INFO , NOTE , PLATE_STATUS , LAST_TIME , LAST_USER  )  VALUES    (  " +  DbTools.setDB ( m_pnrPlate  ) +   " ,  " +  DbTools.setDB ( m_pnrVehicle  ) +   " ,  " +  DbTools.setDB ( m_pnrVehicleCard  ) +   " ,  " +  DbTools.setDB (m_pnrDeliveredInfo  ) +   " ,  " +  DbTools.setDB ( m_note  ) +   " ,  " +  DbTools.setDB ( m_plateStatus  ) +   " ,  " +  DbTools.setDB ( m_lastTime  ) +   " ,  " +  DbTools.setDB ( m_lastUser  ) +   "  ) " ;
          ...
          {
             String SQL =  " INSERT INTO  CURRENT_MOVEMENT    ( PNR , PNR_PLATE , PNR_VEHICLE , PNR_VEHICLE_CARD , PNR_DELIVERED_INFO , NOTE , PLATE_STATUS , LAST_TIME , LAST_USER  )  VALUES    ( " + DbTools.setDB ( m_pnr ) + " ,   " +  DbTools.setDB ( m_pnrPlate  ) +   " ,  " +  DbTools.setDB ( m_pnrVehicle  ) +   " ,  " +  DbTools.setDB ( m_pnrVehicleCard  ) +   " ,  " +  DbTools.setDB (m_pnrDeliveredInfo  ) +   " ,  " +  DbTools.setDB ( m_note  ) +   " ,  " +  DbTools.setDB ( m_plateStatus  ) +   " ,  " +  DbTools.setDB ( m_lastTime  ) +   " ,  " +  DbTools.setDB ( m_lastUser  ) +   "  ) " ;
       ...
       {
           logWriter.write(ex);
          System.out.println( "CurrentMovementRecord .Insert( int type )" + ex.toString()); 
       }
       return false ;
    }
 @Override
    public boolean update( int type )
    {
       ...
       try 
       {
          String SQL =  " UPDATE   CURRENT_MOVEMENT  Set  "  +  " PNR_PLATE =  "  + DbTools.setDB ( m_pnrPlate ) +  " , PNR_VEHICLE =  "  + DbTools.setDB ( m_pnrVehicle ) +  " , PNR_VEHICLE_CARD =  "  + DbTools.setDB ( m_pnrVehicleCard ) +  " , PNR_DELIVERED_INFO =  "  + DbTools.setDB (m_pnrDeliveredInfo ) +  " , NOTE =  "  + DbTools.setDB ( m_note ) +  " , PLATE_STATUS =  "  + DbTools.setDB ( m_plateStatus ) +  " , LAST_TIME =  "  + DbTools.setDB ( m_lastTime ) +  " , LAST_USER =  "  + DbTools.setDB ( m_lastUser )  + " WHERE PNR = " + m_pnr  ;
          return m_connection.executeSQL( SQL ) ;
       }
        catch (Exception ex) 
       {
